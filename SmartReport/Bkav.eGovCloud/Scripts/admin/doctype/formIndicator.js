...
function loadData() {
    ...
    if (defineConfigJson !== '' && defineConfigJson != undefined) {
        var dataConfig = JSON.parse(defineConfigJson)["data"];
        var countRowField = JSON.parse(defineFieldJson)["data"].length;
        //var countRowField = 1;
        dataConfig.forEach(function (val) {
            switch (val[15 + countRowField]) {
                case "KT":
                    val[15 + countRowField] = "Kỳ trước";
                    break;
                case "CK":
                    val[15 + countRowField] = "Cùng kỳ";
                    break;
                case "TC":
                    val[15 + countRowField] = "Tự chọn";
                    break;
                default:
                    break;
            }
            ...
            _dataLocalityDataIds.forEach(function (value, index) {
                if (val[16 + countRowField] !== undefined) {
                    if (val[16 + countRowField] == value) {
                        val[16 + countRowField] = _dataLocalitys[index];
                    }
                }
            });

            _categoryDisaggregationIds.forEach(function (value, index) {
                if (val[17 + countRowField] !== undefined) {
                    if (val[17 + countRowField] == value) {
                        val[17 + countRowField] = _categoryDisaggregationNames[index];
                    }
                }
            });

            if (val.length > 14) {
                if (val[13 + countRowField] == "Indicator") {
                    val[13 + countRowField] = "Chỉ tiêu";
                    switch (val[14 + countRowField]) {
                        case "IndicatorCode":
                            val[14 + countRowField] = "Mã chỉ tiêu";
                            break;
                        case "IndicatorName":
                            val[14 + countRowField] = "Tên chỉ tiêu";
                            break;
                        case "IndicatorUnit":
                            val[14 + countRowField] = "Đơn vị chỉ tiêu"
                            break;
                        default:
                            break;
                    }
                }
                else if (val[13 + countRowField] == "TypeData") {
                    val[13 + countRowField] = "Loại số liệu";
                    var dataTypeNamesArr = JSON.parse($('#dataTypeNames').val());
                    var dataTypesArr = JSON.parse($('#dataTypes').val());
                    dataTypesArr.forEach(function (value, index) {
                        if (val[14 + countRowField] == value) {
                            val[14 + countRowField] = dataTypeNamesArr[index];
                        }
                    })
                } else {
...
function btnSaveTypeConfig(isOnLoad) {
    ...
    if (fieldRowsCount == 1) {
        $.each(typeTableData, function (index, item) {
            if (item[fieldRowsCount] == 'Catalog' && item[fieldRowsCount + 1] != '' && item[fieldRowsCount + 8] && item[fieldRowsCount + 9] != '') {
                ...
                gia_tri_prop.FieldModel = item[fieldRowsCount + 13];
                gia_tri_prop.DataFieldModel = item[fieldRowsCount + 14];
    ...
    dataTable.addHook('afterOnCellMouseUp', function (event, coords, td) {
        localStorage.removeItem("selected");
        localStorage.removeItem("selectedRange");
        var selected = JSON.stringify(coords);
        var selectedRange = JSON.stringify(dataTable.getSelectedRange());
...
function generateTypeTable(isOnLoad) {
    ...
    colHeaders.push('Kiểu dữ liệu', 'Chi tiết', 'Giá trị mặc định', 'Bắt buộc', 'Chỉ đọc', 'Phạm vi', 'Ghi đè', "Ẩn cột desktop", "Xoay", "Giá trị", "Ẩn cột mobile", "Inline", "Auto size", "Loại bảng", "Giá trị bảng", "Kỳ báo cáo", "Địa bàn", "Phân tổ");
    ...
    if (typeColumsSetting.length == 0) {
        ...
        typeColumsSetting.push({
            type: 'dropdown',
            source: ["","Tự chọn", "Kỳ trước", "Cùng kỳ"]
        });
        //Địa bàn
        typeColumsSetting.push({
            type: "dropdown"
    ...
    typeTable.addHook('beforeChange', function (changes, source) {
        ...
        if (typeTable.getColHeader(col) == 'Loại bảng') {
            switch (value_after) {
                case 'Chỉ tiêu':
                    propIncatalog = { source: ['Mã chỉ tiêu', 'Tên chỉ tiêu', 'Đơn vị chỉ tiêu'], type: 'dropdown', readOnly: false }
                    typeTable.setCellMetaObject(row, col + 1, propIncatalog);
                    typeTable.setCellMeta(row, col + 1, "renderer", "dropdown");
                    break;
                case 'Loại số liệu':
                    cate_dropdown = JSON.parse($('#dataTypeNames').val());
                    propTypedata = { source: cate_dropdown, type: 'dropdown', readOnly: false }
                    typeTable.setCellMetaObject(row, col + 1, propTypedata);
                    typeTable.setCellMeta(row, col + 1, "renderer", "dropdown");
                    break;
                default:
                    propEmpty = { source: [''], type: 'dropdown', readOnly: false }
                    typeTable.setCellMetaObject(row, col + 1, propEmpty);
                    typeTable.setCellMeta(row, col + 1, "renderer", "dropdown");
                    break;
            }
        }

        if (typeTable.getColHeader(col) == 'Địa bàn') {
                var locality_dropdown = JSON.parse($('#dataLocalitys').val());
                propData = { source: locality_dropdown, type: 'dropdown', readOnly: false }
                typeTable.setCellMetaObject(row, col, propData);
                typeTable.setCellMeta(row, col, "renderer", "dropdown");
        }

        if (typeTable.getColHeader(col) == 'Phân tổ') {
            var categoryDisaggregationNames_dropdown = JSON.parse($('#categoryDisaggregationNames').val());
            propData = { source: categoryDisaggregationNames_dropdown, type: 'dropdown', readOnly: false }
            typeTable.setCellMetaObject(row, col, propData);
            typeTable.setCellMeta(row, col, "renderer", "dropdown");
        }
    ...
    typeTable.updateSettings({
        cells: function (crow, ccol, prop) {
            ...
            //if (isEdit) {
            //    if (ccol === 14 || ccol === 15 || ccol ===16) {
            //        cellProperties.className = 'gray htLeft',
            //        cellProperties.readOnly = true
            //    }
            //}
            return cellProperties;
...
//    td.innerHTML = '<div title="' + td.innerHTML + '">' + td.innerHTML.slice(0, 64) + '</div>';

//    if (instance.getDataAtCell(row, col).length > 2) {

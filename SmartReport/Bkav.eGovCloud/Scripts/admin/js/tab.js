...
function generateFieldTable() {
    ...
    fieldTable.addHook('afterCreateCol', function () {
        console.log('da them cot');
    })

    fieldTable.addHook('beforeCreateCol', function () {
        console.log('da them cot 2');
    })

    fieldTable.addHook('beforeColumnMove', function (movedColumns, finalIndex, dropIndex, movePossible) {
        isClickTab2 = true;
        isClickTab3 = true;
    })
    fieldTable.addHook('afterColumnResize', function (movedColumns, finalIndex, dropIndex, movePossible, orderChanged) {
        isClickTab3 = true;
    })
    

    fieldTable.addHook("afterChange", function (changes, source) {
        ...
        contextMenu: {
            items: {
                ...
                "make_strike": {
                    ...
                    callback: function (key, options) {
                        setClassNameByRange(this, this.getSelectedRange(), 'htUnderline');
                        this.render();
                    }
                }
            }
...
function generateTypeTable(isOnLoad) {
    ...
    if (typeColumsSetting.length == 0) {
        ...
        typeColumsSetting.push({ type: 'checkbox' }, { type: 'checkbox' }, { className: "inputMasked", editor: inputMaskedEditor })

        // Ghi đè
        typeColumsSetting.push({ type: 'checkbox' });
    ...
    typeTable.addHook('beforeChange', function () {
        isClickTab3 = true;
    })
            ...
            switch (value_after) {
                ...
                default:
                    if (typeTable.getDataAtCell(row, col - 1) == 'Catalog' && value_after != '') {
                        ...
                        $.getJSON('/Admin/Form/GetCatalogValues2', { catalogName: value_after, row: row, col: col, formId: formId, isXoay: false },
                        ...
                        var formId = $('#Form_FormId').val() != null ? $('#Form_FormId').val() : $('#FormId').val();
                        $.getJSON('/Admin/DocType/GetInCatalogValues2', { inCatalogName: value_after, row: row, col: col, formId: formId, isXoay: false },
                            function (data, textStatus, jqXHR) {
    ...
    for (var i = 0; i < fieldColumnsCount; i++) {
        if (!isOnLoad) {
            ...
            if (typeTableData[i][fieldRowsCount] == "Catalog") {
                ...
                var formId = $('#Form_FormId').val() != null ? $('#Form_FormId').val() : $('#FormId').val();
                $.getJSON('/Admin/Form/GetCatalogValues2', { catalogName: typeTableData[i][fieldRowsCount + 1], row: i, col: (fieldRowsCount + 1), formId: formId, isXoay: false },
                    function (data, textStatus, jqXHR) {
                        var catalogValues = JSON.parse(data.catalogValues);
                        propDefault = { type: 'dropdown', source: catalogValues, readOnly: false }
                ...
                var formId = $('#Form_FormId').val() != null ? $('#Form_FormId').val() : $('#FormId').val();
                $.getJSON('/Admin/DocType/GetInCatalogValues2', { inCatalogName: typeTableData[i][fieldRowsCount + 1], row: i, col: (fieldRowsCount + 1), formId: formId, isXoay: false },
                    function (data, textStatus, jqXHR) {
                        var catalogValues = JSON.parse(data.inCatalogValues);
                        propDefault = { type: 'dropdown', source: catalogValues, readOnly: false }
                        typeTable.setCellMetaObject(data.row, fieldRowsCount + 2, propDefault);
...
/* event click*/
var isClickBtnLeft = true;
$("#btn-toggle-main-left").click(function () {
    if (isClickBtnLeft) {
        hideTabRight();
    } else {
        $("#home2 .modal-body").css('width', '90%');
        showTabRight();
    }
})
function hideTabRight() {
    isClickBtnLeft = false;
    $("#left-main").hide();
    $("#home2 .modal-body").css('width', '100%');
    $("#adddivtabdiv").removeClass("col-md-9");
    $("#adddivtabdiv").addClass("col-md-12");
}
function showTabRight() {
    isClickBtnLeft = true;
    $("#left-main").show(800);
    $("#adddivtabdiv").removeClass("col-md-12");
    $("#adddivtabdiv").addClass("col-md-9");
}
$("button.tab1").click(function () {
    $("#content-tab2-footer").hide();
    $("#content-tab1-footer").toggle(1000);
});
$("button.tab2").click(function () {
    $("#content-tab1-footer").hide();
    $("#content-tab2-footer").toggle(1000);
});
/*end event click*/
(function (eR, $) {
            ...
            addFiedPanel.find("[type='button']").click(function () {
                if ($(this).hasClass("cancel")) {
                    ...
                    var newItem = $("<li>").append(
                            $('<div>').addClass(config.toolbox.field_className)
                            .attr('value', value)
                            .attr('datatype', datatype).text(text));
    ...
    var _editField = function () {
        $(config.toolbox.field_class).each(function () {
            $(this).dblclick(function () {
                editField(this);
            });
        })

        var editField = function (_this) {
            $(config.contentPanel.jClass).find(config.toolboxPanel.addField_class).remove();
            $(config.contentPanel.jClass).append(template.addField);
            var addFiedPanel = $(config.contentPanel.jClass).find(config.toolboxPanel.addField_class);
            addFiedPanel.find(":text").first().focus();
            ...
            var oldeValue = $(_this).attr('value');
            addFiedPanel.find(":text.name").val($(_this).text());
            addFiedPanel.find(":text.value").val(oldeValue);
            addFiedPanel.find(".datatype").val($(_this).attr('datatype'));
            ...
            addFiedPanel.find("[type='button']").click(function () {
                if ($(this).hasClass("cancel")) {
                    ...
                    //chi tiết
                    var usedDetail = $(config.contentPanel.content_class).find(config.contentPanel.used_field + ":contains('" + oldeValue + "')");
                    if (usedDetail.length > 0) {
                        $(usedDetail).text("[" + value + "]")
                            .attr('datatype', datatype);
                    }
                    var usedHeader = $(config.table.header_class).find(config.contentPanel.used_field + ":contains('" + oldeValue + "')");
                    if (used.length > 0) {
                        $(used).text("[" + value + "]")
                            .attr('datatype', datatype);
                    }
    ...
    var _deleteField = function () {
        ...
        $(config.toolbox.deleteField_class).click(function () {
            ...
            var used = $(config.contentPanel.content_class).find(config.contentPanel.used_field + ":contains('" + fieldValue + "')");
            if (used.length > 0) {
    ...
    var _clickToPanel = function (_this) {
        ...
        .mousedown(function (e) {
            ...
            if ($(e.target).hasClass(config.toolbox.field_className)
                || $(e.target).is("td, th")
                || $(e.target).parents("td, th").length > 0) {
                return;
            }
    ...
    var _bindSelectable = function (_this) {
        ...
        $(config.contentPanel.content_class).mousedown(function (e) {
            ...
            if ($(e.target).hasClass(config.toolbox.field_className)
                || $(e.target).is("td, th") || $(e.target).parents("td, th").length > 0) {

                if ($(e.target).is("td, th") || $(e.target).parents("td, th").length > 0) {
                    $(config.contentPanel.content_class).find("td, th," + config.toolbox.field_class)
                        .removeClass(config.contentPanel.textSelected_className);
                }
                return;
            }
            isMouseDown = true;

(function (egov, $, _, undefined) {
    ...
    egov.utilities.checkbox.checkAndUnCheckAll = function (allitems, item) {
    ...
    egov.utilities.jstree.bindJsTree = function (divTree, hasUser, hasCheckbox, hasDnD, arrDept, arrUsers, arrDeptUserJobtitles, callBack, dataBind) {
        ...
        if (deptRoot) {
            divTree.jstree({
                ...
                divTree.bind("open_node.jstree", function (event, data) {
                    if (data.inst._get_children(data.rslt.obj).length == 0) {
                        egov.utilities.jstree.appendChild(data.rslt.obj, parseInt(data.rslt.obj.attr("id")), hasUser, hasCheckbox, arrDept, arrUsers, arrDeptUserJobtitles);
                    }
                });
            ...
            setTimeout(function () { $.jstree._focused().select_node("#" + deptRoot.value); }, 200);
        }
    };

    egov.utilities.jstree.appendChild = function ($parent, parentId, hasUser, hasCheckbox, arrDept, arrUsers, arrDeptUserJobtitles) {

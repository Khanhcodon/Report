define([
    ...
    var DocumentsView = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            that.nodeId = that.isStoreTree ? that.node.model.get('storePrivateId')
                                            : that.node.model.get('funtionIdNote') ? that.node.model.get('funtionIdNote') : that.node.model.get('functionId');
        ...
        getListDocumentByLevelAction: function (listDocument) {
            var $levels = $("#typeListRow").find(".panel.panel-default");
            var documents = listDocument;
            //$levels.each(function () {
            //    var levelActionId = $(this).attr("data-id");
            //    var docs = _.filter(listDocument, function (itemDocument) {
            //        return itemDocument.ActionLevel == levelActionId;
            //    });

            //    //$(this).find(".badged").text("(" + docs.length + ")");
            //    $(this).find(".badged").text(docs.length);
                        ...
                        documentContextmenu.getContextmenu(selectedIds, selectedModel, target, function (items) {
                            ...
                            var command = function (items, theoLoActions) {
                                ...
                                contextData.add({
                                    ...
                                    iconClass: "icon-info",
                                    callback: function () {
                                        that.showDetailBmm(selectedIds);
                                    }
                                });
                                //Đính chính
                                contextData.add({
                                    key: "dinhchinh",
                                    text: "Đính chính",
                                    iconClass: "icon icon-forward4",
                                    callback: function () {
                                        that.showDinhChinh(selectedIds);
                                    }
                                });
                                // Xóa dữ liệu
                                contextData.add({
                                    key: "xoadulieu",
                                    text: "Xóa dữ liệu",
                                    iconClass: "icon icon-remove2",
                                    callback: function () {
                                        that.showXoaDuLieu(selectedIds);
                                    }
                                });
            ...
            if (egov.setting.user.userSetting.isFullQuickView) {
                ...
                require(['documentView'], function (DocumentView) {
                    ...
                    var documentView = new DocumentView({
                        ...
                        survey: data.CategoryBusinessId == 32
                    });
        ...
        _getLastestDocuments: function (callback, isAuto) {
            ...
            egov.request.getLastestReports({
                data: data,
                success: function (result) {
                    if (!result || (!result.documents || typeof result.documents !== 'object')) {
                        return;
                    }
                    result.documents = that.getListDocumentByLevelAction(result.documents)
        ...
        _bindCreateReportRow: function () {
            var nodeParam = JSON.parse(this.node.model.get("params"));
            ...
            var result = { "Status": "Chưa nhập", "Compendium": "", "Compendium2": "", "DateCreated": "17/08/2019", "DocTypeId": "", "CategoryBusinessId": 4, "DocumentId": "0", "DatePublished": "", "DateReceived1": "2019-08-17T14:43:55", "DocumentCopyId": 0, "hasUpdateViewed": 0, "ActionLevel": 1 };
            ...
            switch (actionLevel) {
                case 1: // Year
                    ...
                    break;
                case 2: // 6 Tháng
                    result.DatePublished = "6T " + (now.getMonth() < 6 ? "đầu năm" : "cuối năm");
                    break;
                case 3: // Quarter
        ...
        showXoaDuLieu: function (selected) {
            var that = this;
            var organization = this.model.get("Organization");
            var dinhChinhShow = "<div>Bạn có muốn xóa dữ liệu này không?</div>"
                ...
                buttons: [
                        {
                            ...
                            click: function () {
                                ...
                                //egov.request.dinhchinh({
                                //    data: {
                                //        docCopyId: selected[0]
                                //    },
                                //    success: function (res) {
                                //        if (res.error) {
                                //            egov.pubsub.publish(egov.events.status.error, res.message);
                                //        } else {
                                //            egov.pubsub.publish(egov.events.status.success, res.message);
                                //            that.openDocument();
                                ...
                                //    data: {
                                //        documentCopyId: selected[0]
                                //    },
                                //    success: function(res){
                                //        var actionArr = $.grep(res, function (v) {
                                //            return v.id === "ChuyenNguoiKhoiTao";
                                //        });
                                //        alert(JSON.stringify(actionArr))
                                //    }
                                //})
                                //that.document._dinhchinh();
                                $el.dialog('destroy');
                            }
                        },
                        {
        ...
        showDinhChinh: function (selected) {
            var that = this;
            var dinhChinhShow = "<div>Bạn có muốn đính chính</div>"
            //var bmmDialog = "<div style='width: 100%; height: 100%;'><iframe style='width: 100%; height: 100%; border:none;' src='http://quanlynhiemvu.langson.gov.vn/Views/CreateMission.aspx?callBackSuccessful=GetListMissions'></iframe><div>";
            var $el = $(dinhChinhShow);
            $el.dialog({
                title: "Đính chính",
        ...
        showDetailBmm: function (selecteds) {
            egov.request.LinkDetailMission({
                data: {
                    docIds: selecteds
                },
                success: function (data) {
                    if (data.error) {
                        ...
                        var bmmDetailDialog = "<div style='width: 100%; height: 100%;'><iframe style='width:100%;height:400px;' src='" + data.url + "'></iframe></div>";
                        var $el = $(bmmDetailDialog);
                        $el.dialog({
        ...
        showBmm: function (selecteds) {
            //lấy thông tin user
            var that = this;
            egov.request.GetListUser({
                data: {
                    docId: selecteds
                },
                success: function (data) {
                    if (data.isOk) {
                        that._missionRender(selecteds, data.Object);
                    } else {
                    }
                },
                error: function () {
        ...
        _missionRender: function (selecteds, data) {
            var MissionError = function (message) {
                egov.pubsub.publish(egov.events.status.error, message);
            };

            var bmmDialog = "<div style='width: 100%; height: 100%;'><form id ='formBmm'>"
                ...
                + '</div>'
                + '<div class="form-group" style="border: 1px solid #BDBDBD;padding:5px;overflow:scroll; height:250px;">'
                + this._renderMissionUser(data)
                + '</div>'
                + '</div>'
                + '</div>'
                + "</form><div>";
            ...
            var $el = $(bmmDialog);
            var textContent = $el.find('#txtContent');

            var missionDate = $el.find('#MissionDate');
            var username = $el.find('#txtUsername');
            var coUsername = $el.find('#txtCoUsername');
                ...
                buttons: [
                    {
                        ...
                        click: function () {
                            ...
                            if (content === '' || content == null) {
                                $(textContent).addClass('input-validation-error').first().focus();
                                return;
                            }
                            if (date === '' || date == null) {
                                $(missionDate).addClass('input-validation-error').first().focus();
                                return;
                            }

                            if (user === '' || user == null) {
                                MissionError('Chưa chọn người xử lý!');
                                return;
                            }
                            var coUser = [];
                            ...
                            egov.request.CreateMission({
                                ...
                                success: function (data) {
                                    if (data.success) {
                                        $el.dialog('destroy');
                                        egov.pubsub.publish(egov.events.status.success, 'Tạo nhiệm vụ thành công.');
                                    } else {
                                        MissionError(data.error);
                                    }
                                },
                                error: function () {
                                    console.log('error');
            ...
            $el.find('#formBmm .mission-main-process').click(function (e) {
                var user = $(username).val();
                if (user) {
                    $('#formBmm .mission-main-process [data=' + user + ']').prop('checked', false);
                    $('#formBmm .mission-co-process [data=' + user + ']').prop('disabled', false);
                }
                if (e.target.checked) {
                    user = $(this).attr('data');
                    $(username).val(user);
                    $('#formBmm .mission-co-process [data=' + user + ']').prop('disabled', true);
                    $('#formBmm .mission-co-process [data=' + user + ']').prop('checked', false);
                } else {
                    $(username).val('');
                }
            });
            ...
            if (users) {
                users.forEach(function (item) {
                    result = result + '<div class="form-group" style="border-bottom: 1px solid #BDBDBD;">'
                        ...
                        + '<input name="checkbox[]" type="checkbox" data="' + item.UserName + '">'
                        ...
                        + '<input name="cocheckbox" type="checkbox" value="' + item.UserName + '" data="' + item.UserName + '">'
                        + '<span class="document-color-1"><i class="icon-check"></i></span>'
                        + ' </label>'
                        + '</div>'
        ...
        showCalendar: function () {
            var calendarDialog = "<div style='width: 100%; height: 100%;'><iframe style='width: 100%; height: 100%; border:none;' src='http://quanlynhiemvu.langson.gov.vn/Views/CreateMission.aspx?callBackSuccessful=GetListMissions'></iframe><div>";
    ...
    var DocumentItem = Backbone.View.extend({
        ...
        events: {
            ...
            'click .btn-createnew': 'createDocument'
        },
        ...
        renderDefault: function () {
            ...
            var isCreateRow = this.model.get('DocumentCopyId') == 0;
            ...
            $.each(this.columnConfig, function (index, setting) {
                ...
                if (setting.ColumnName === 'Status') {
                    tdElement.css("color", isCreateRow ? "red" : "blue");
                }

                if (setting.ColumnName === 'DateCreated' && isCreateRow) {
                    tdElement.html("<button class='btn btn-danger btn-createnew btn-xs' data-id='" + that.model.get("DocTypeId") + "' data-name='" + that.model.get("Compendium") + "'>Nhập báo cáo</button>");
                }
        ...
        createDocument: function (e) {
            var target = $(e.target).closest(".btn-createnew");
            var doctypeId = target.attr("data-id");
            var compendium = target.attr("data-name");

            egov.views.home.tab.addDocument(doctypeId, compendium);
        },
        ...
        dbclick: function () {
            ...
            if (this.model.get("IsFile")) {
                ...
                this.$el.removeClass('documentUnread');
                this.model.set("IsViewed", 1);
                if (this.model.get('IsViewed')) {
                    this.$el.removeClass('documentUnread');
                }
                that.$el.removeClass(that.unreadClass);

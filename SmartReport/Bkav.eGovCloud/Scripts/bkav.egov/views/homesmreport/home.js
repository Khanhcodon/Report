define(function () {
    ...
    var HomeView = Backbone.View.extend({
        ...
        events: {
            ...
            'keyup .search-query': 'seachReport',
            'click #searchMenuNav': '_doSearch',
            'click .btn-bars': '_doBars'
        },
        ...
        seachReport: function (e) {
            if (e.keyCode === 13) {
                this._doSearch();
            }
            this._filterDocumentInClient($(".search-query").val());
        },
        _filterDocumentInClient: function(filterValue) {
            ///<summary>
            /// Lọc danh sách văn bản hiện tại khi gõ vào ô tìm kiếm.
            ///</summary>

            egov.views.home.documents.clientQuickSearch(filterValue);
        },

        _doSearch: function () {
            egov.views.home.tab.addSearch("", 1);
        },
        _doBars: function () {
            if (!$(".bars-left").hasClass("hidden-bar")) {
                $(".bars-left").addClass("hidden-bar");
                $(".bars-left").removeClass("col-md-3");
        ...
        renderDefault: function (callback) {
            ...
            require(['tabReportView'], function (TabView) {
                // Render ra các tab
                that.tab = new TabView;
                // Render ra tree
                require(['treeReportView'], function (TreeView) {

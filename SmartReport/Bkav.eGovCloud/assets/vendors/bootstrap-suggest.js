...
(function ($) {
    ...
    Suggest.prototype = {
        ...
        __keyup: function (e) {
            ...
            var idxCursor = $('#txtCustomField').getCursorPosition();
            if ($.inArray(e.keyCode, specialChars) !== -1) return;
            var $el = this.$element,
                val = $el.val().substring(0, idxCursor)
                    .split(" ")
                    .pop().split("(")
                    .pop().split(")")
                    .pop().split("+")
        ...
        __select: function (index) {
            var $el = this.$element,
                ...
                val = $el.val(),
                item = this.get(index),
                setCaretPos = this._keyPos + item.value.length - 2,
                idxCursor = $('#txtCustomField').getCursorPosition();
            var aa = val.substring(0, idxCursor);
            var ab = aa.split(" ")
                .pop().split("(")
                .pop().split(")")
                .pop().split("+")
                ...
                .pop().split("%")
                .pop();
            aa = aa.substring(0, aa.lastIndexOf(ab));

            $el.val((aa + item.value + val.substring(idxCursor)).trim());

            val = $('#txtCustomField').val();
            if (val.endsWith('()'))
                setCaretPos = val.length - 1;
            else

...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `mobile_documentprocessings`(IN `@userId` int,IN `@categoryBusinessId` int, IN `@viewed` bool)
BEGIN
	#Routine body goes here...
	SELECT 
			`d`.`DocumentId`, 
	...
	WHERE `dc`.`Status` = 2 
		...
		AND `dc`.`DocumentCopyType` in (1, 2, 4, 32, 64)
		AND (`@viewed` is null 
				OR (`@viewed` = 0 AND INSTR(dc.UserNguoiDaXem, CONCAT(';',`@userId`,';')) = 0 OR INSTR(dc.UserNguoiDaXem, CONCAT(';',`@userId`,';')) is null)
				OR (`@viewed` = 1 AND INSTR(dc.UserNguoiDaXem, CONCAT(';',`@userId`,';')) > 0))
	ORDER BY `dc`.`DateReceived` DESC;
END
;;
DELIMITER ;

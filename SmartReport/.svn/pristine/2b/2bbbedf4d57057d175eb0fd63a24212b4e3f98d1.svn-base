...
+function ($) {
  ...
  Tree.prototype.collapse = function (tree, parentLi) {
    ...
    // Private
  var clickElement = $('#reportTree').find(".treeview a .btnToggleTree");
  ...
  //      debugger
  //      if ($(event.target).closest(".btnToggleTree").length) {
  //          var parentLi = $(event.target).closest("li");
  //          var link = $(event.target).closest("a");
  //          if (!parentLi.hasClass("is-loader")) {
  //              var idParent = $(link).attr("data-id")
  //              window.GetReportsByParent(idParent, link.next());
  //              parentLi.addClass("is-loader");
  //          }
  //          that.toggle($(this).parent(), event)
  //      } else {
  //          debugger
  //          window.ShowReport($(event.target).closest("a"))
  //      }
  //  });
    //}
  ...
  Tree.prototype._setUpListeners = function () {
      ...
      $('#reportTree').on('click', '.treeview', function (event) {
          $('#yearSelect option:first').prop('selected', true);
          $('.tree').find('.treeview a').removeClass('activeBgOld');
          var countBtn = $(event.target).closest(".treeview a").length;
          var lengthBtnSpan = $(event.target).closest(".treeview a").find('.btnToggleTree').length;
          var $ev = $(event.target).closest('.treeview');
          $ev.siblings().removeClass('activeBg');
          $ev.removeClass('activeBgOld');
          if (countBtn) {
              if (lengthBtnSpan) {
                  var parentLi = $(event.target).closest("li");
                  var link = $(event.target).closest("a");
                  ...
                  that.toggle($(this).parent(), event);
                  //that.toggle($(this).find('a'), event);

                  $ev.find('.treeview-menu').find('.treeview').removeClass('activeBg');
                  $ev.find('.treeview-menu').find('.treeview').removeClass('activeBgOld');
                  $(event.target).closest('.treeview-menu').closest('.treeview')
                      .closest('.treeview-menu').closest('.treeview')
                      .closest('.treeview-menu').closest('.treeview')
                      .find('a').removeClass('activeBgOld');
                  $(event.target).closest('.treeview-menu').closest('.treeview')
                      .closest('.treeview-menu').closest('.treeview')
                      .find('a').removeClass('activeBgOld');
                  $(event.target).closest('.treeview-menu').closest('.treeview')
                  ...
                  $ev.addClass('activeBg');
                  var test = $(event.target).closest('.treeview-menu').closest('.treeview')
                      .closest('.treeview-menu').closest('.treeview')
                      .closest('.treeview-menu').closest('.treeview')
                      .find('a').addClass('activeBgOld');
                  $(event.target).closest('.treeview-menu').closest('.treeview')
                      .closest('.treeview-menu').closest('.treeview')
                      .find('a').addClass('activeBgOld');
                  $(event.target).closest('.treeview-menu').closest('.treeview')

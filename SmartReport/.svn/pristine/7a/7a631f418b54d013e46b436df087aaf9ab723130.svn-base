...
(function (eR) {
    ...
    imageField.prototype.init = function () {
        ...
        result.append($("<img>").addClass("img-class").addClass("no-image")
            .attr("src", config.tmp.img_noImage_src));
        ...
        $($this).find("th, td")
            ...
            .droppable({
                ...
                drop: function (event, ui) {
                    ...
                    else if (ui.helper.attr("type") == "sort") {
                        _addControlSort($(this));
                    }
                    else {
    ...
    var _showProperties = function (obj) {
        ...
        var setProperties = function (prop1) {
            ...
            if (property1.is("select")) {
                var itm = [];
                try {
                    itm = property1.find("option[value~='" + value1 + "']");
                } catch (e) {
    ...
    var _addControlSort = function ($this) {
        var addControl = $(template.addControlSort);
        addControl.draggable();
        addControl.css("position", "");
        addControl.find("[type='button']").click(function () {
            if ($(this).hasClass("cancel")) {
            ...
            var newItem = $("<div>").addClass(config.toolbox.field_className)
                            .attr('data-value', sortBy)
                            .attr("data-hassort", true)
                            .attr("data-link", true)
                            .text(name);
    ...
    var _showSortValue = function () {
        $(".sort-value").show();
        $(".field-collection").append($("<option>").text("Chọn").val());

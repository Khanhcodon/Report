...
----------------------------------------------------------------------------
  ...
  last modify       : Thu, 28 Nov 2012 16:20:22 +0700
  version           : 1.0
----------------------------------------------------------------------------
...
//FIXME: I need a control to enable/disable displaying log in this object
...
if (typeof(console)=='undefined') {	//if console doesnt exist, use the annoying alert instead
    console=function () {
        return this;
    };	// console.debug   <--- instruct the build script remove this line
...
CHIM.GetTarget = function(e) {
	...
	//console.log(r.tagName + " "  + r.type);
    CHIM.peckable = r.tagName == 'HTML' || r.type == 'textarea' || r.type == 'text' || r.tagName == 'DIV';//fix for div with contenteditable attr is true
...
CHIM.GetCursorPosition = function( target ) {
	if (target == null || target.value == null || target.value.length == 0) {
		...
		//if it's a div tag
			...
			//change to innerHTML <->textContent
			return target.innerHTML.length;
		}else{
			return -1;
		}
   	 }
...
/**
 *@param target target that is processed
 *@param p position
 */
CHIM.SetCursorPosition = function(target, p) {
    ...
    if(target.value){
        ...
        /**
         *In most browsers, you need the Range and Selection objects. 
         *You specify each of the selection boundaries as a node and an offset 
         *within that node. For example, to set the caret to the fifth character
         *of the second line of text, you'd do the following:
	    ...
	    var sel = window.getSelection();
            for(i = 0; i < target.childNodes.length - 1; i++){
                //console.log("ducla " + i + ":" + target.childNodes.item(i).nodeValue);
            }
            //console.log("50:" + target.innerHTML + ":" + target.childNodes.length + ":" + i);
	    //range.setStart(target.childNodes.item(0), p);
	    //console.log("4 " + target.childNodes.item(target.childNodes.length - 1).nodeValue);
	    //console.log("5: "  + p + " " + target.childNodes.item(target.childNodes.length - 1).nodeValue.length);//FIXME: if we process for a child node, p is not right anymore
	    //console.log("6: " + target.innerHTML);
	    //range.setStart(target.childNodes.item(target.childNodes.length - 1), p);
//            console.log((i + 1) + ": " + target.childNodes.item(i).nodeValue);
	    range.setStart(target.childNodes.item(i), target.childNodes.item(i).nodeValue.length);
	    range.collapse(true);
	    sel.removeAllRanges();
	    sel.addRange(range);
	    target.focus();
...
CHIM.UpdateBuffer = function(target) {
    ...
    if (target.tagName != 'HTML') {
		...
		if(target.value){
			...
			while ( c >= 0 && separators.indexOf(target.textContent.charAt(c)) < 0 ) {
				CHIM.buffer.unshift(target.textContent.charAt(c));
				c = c - 1;
		    	}
		}
        }
...
CHIM.KeyHandler = function(e) {
	//console.log("keyhanlder:" + e.keyCode);
...
//	console.log(CHIM.GetTarget(e) + " " + CHIM.peckable + " " + CHIM.Freeze(target));
        return;
    }
...
//            console.debug('Temporarily disable spell checking');
    ...
    if((e.keyCode == Mudim.key1) && Mudim.redefineKey){
        CHIM.keyCombine[Mudim.key1] = false;
    }
...
//	console.log("keydown:" + e.keyCode);
    ...
    if(!Mudim.redefineKey){
        ...
        if(e.keyCode == Mudim.key1){
            CHIM.keyCombine[Mudim.key1] = true;
        }
        if((e.keyCode == Mudim.key2) && CHIM.keyCombine[Mudim.key1]){
           CHIM.Toggle();
           return;
        }
    }
...
/**
 * For press then release mouse
 *@param e
 */
CHIM.OnClick = function(e){
    //CHIM.getCargetPosition();
    //return -1;
}
Mudim.tempPosition = 0;

CHIM.getCargetPosition = function(){
    if(window.getSelection && window.getSelection().getRangeAt){
        ...
        for(var i = 0; i < childNodes.length; i++){
            ...
            if(childNodes[i].outerHTML){
                ...
                //console.log("html: " + childNodes[i].outerHTML);
                //console.log(rangeCount);
            }
            else if(childNodes[i].nodeType == 3){
                rangeCount += childNodes[i].textContent.length;
                //console.log("text:" + childNodes[i].textContent);
                //console.log(rangeCount);
            }
        }
        ...
        var index = range.startOffset + rangeCount;
        //console.log("all: " + index);
        CHIM.tempPosition = index;
        return;
    }
    //return -1;
    CHIM.tempPosition = -1;
...
CHIM.Attach = function(e, r) {
    ...
    if (!e.chim) {
		...
		//console.log("IE");
                ...
                e.attachEvent('onclick', CHIM.OnClick);
		//console.log("added");
            }
            else { // Moz & others
		//console.log("Moz");
                ...
                e.addEventListener('click', CHIM.OnClick, false);
		//console.log("added");
            }
        }
        else {
    ...
    f = e.getElementsByClassName('editor-iframe');
    for (i = 0; i < f.length; i++) {
        //var doc = (!window.opera && document.all) ?
        //f[i].document : f[i].contentDocument;
	doc = f[i];
	//alert(doc);
        try {
            //doc = f[i];           
...
Mudim.UpdateUI = function(target,l) {
	...
	if(target.value){
		...
		//FIXME: if it's a html string, how we can process?
		target.innerHTML = Mudim.HTMLprocess(target, b, start, end);//we set the target's content in here
                //and in here we will define the cursor's position
		CHIM.SetCursorPosition( target, start + b.length);//only set cursor 's position
		target.scrollTop = t;
	}
};

/**
 *@param target div tag
 *@param buffer
 *@param start
 *@param end
 * must insert after a tag, so we must process start point
 */
Mudim.HTMLprocess = function(target, buffer, start, end){
    //console.log("8:" + target.innerHTML + ":" + CHIM.tempPosition);
    //console.log("9: " + target.innerHTML.substring(start - 3, start + 1));
    //range = document.createRange();
    var selectedObj = window.getSelection();
    //console.log("anchornode: " + target.anchorNode.nodeValue);
//    console.log(target.innerHTML);
    for(var i = 0; i < target.childNodes.length; i++){
//        console.log("ducla1 " + i + ":" + target.childNodes.item(i).nodeName + ":" + target.childNodes.item(i).nodeValue);
    }
    var tmp;    
    var index = buffer.toString().replace(/,/g, '').length;
//    console.log(index);
    tmp = target.innerHTML.substring(0, start - index) + buffer.toString().replace(/,/g, '') + target.innerHTML.substring(end);
//    console.log("10: " + tmp);
    return tmp;
};
...
Mudim.InitPanel = function() {
    if (!Mudim.Panel) {
        Mudim.GetPreference();
        Mudim.panels =['<div id="mudimPanel" style="position: fixed; bottom: 0; right:0; left:0; width: 100%; border: 1px solid black; padding: 1px; background: '+Mudim.PANEL_BACKGROUND+'; color:'+Mudim.COLOR+'; z-index:4000; text-align: center; font-size: 10pt;"><a href="http://mudim.googlecode.com" title="Mudzot\'s Input Method" onclick="Mudim.ToggleDisplayMode();return false;">Mudim</a> v0.9 <input name="mudim" id="mudim-off" onclick="Mudim.SetMethod(0);" type="radio">'+Mudim.LANG[0]+'<input name="mudim" id="mudim-vni" onclick="Mudim.SetMethod(1);" type="radio"> '+Mudim.LANG[1]+' <input name="mudim" id="mudim-telex" onclick="Mudim.SetMethod(2);" type="radio"> '+Mudim.LANG[2]+' <input name="mudim" id="mudim-viqr" onclick="Mudim.SetMethod(3);" type="radio"> '+Mudim.LANG[3]+' <input name="mudim" id="mudim-mix" onclick="Mudim.SetMethod(4);" type="radio"> '+Mudim.LANG[4]+' <input name="mudim" id="mudim-auto" onclick="Mudim.SetMethod(5);" type="radio"> '+Mudim.LANG[5]+' <input id="mudim-checkspell" onclick="javascript:Mudim.ToggleSpeller();" type="checkbox">'+Mudim.LANG[6]+'<input id="mudim-accentrule" onclick="javascript:Mudim.ToggleAccentRule();" type="checkbox">'+Mudim.LANG[7]+' [&nbsp;<a href="#" onclick="Mudim.Toggle();return false;">'+Mudim.LANG[8]+'</a> (F9) <a href="#" onclick="Mudim.TogglePanel();return false;">'+Mudim.LANG[9]+'</a> (F8) ]</div>',
...
Mudim.key1 = -1;
Mudim.key2 = -1;
Mudim.redefineKey = false;
/**
 * Redefine key for toggle vk
 * @param key1 the first key (CHIM.VK_xxx)
 * @param key2 the second key (CHIM.VK_xxx)
 */
Mudim.shortKeyDef = function(key1, key2){
    Mudim.key1 = key1;
    Mudim.key2 = key2;
    Mudim.redefineKey = true;
}
//----------------------------------------------------------------------------
/**
* Define method
...
*/
Mudim.LANG=['Tat','VNI','Telex','Viqr','Tong hop','Tu dong','Chinh ta','Bo dau kieu moi','Ba�t/Tat','An/Hien'];
/**
*Array containing ID of elements which doesn't need Vietnamese typing
*/
Mudim.IGNORE_ID = ['textToFromInput1', 'textToFromInput2', 'textToFromInput3', 'textToFromInput4', 'textToFromInput5', 'textToFromInput6', 'textToFromInput7',
'textFromCcInput1', 'textFromCcInput2', 'textFromCcInput3', 'textFromCcInput4', 'textFromCcInput5', 'textFromCcInput6', 'textFromCcInput7',
'textFromBccInput1', 'textFromBccInput2', 'textFromBccInput3', 'textFromBccInput4', 'textFromBccInput5', 'textFromBccInput6', 'textFromBccInput7'];
/**
...
//Changes in 1.0
//support div tag with contenteditable enable

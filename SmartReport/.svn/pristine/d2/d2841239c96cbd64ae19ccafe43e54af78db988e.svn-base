...
//them bien nay de check khi xung dot voi unikey
CHIM.unikey = false;
    ...
    if (CHIM.Speller.enabled && !Mudim.tempDisableSpellCheck) {
        ...
        if (grp > 0 && CHIM.off == 0) {
            ...
            if (len == 0) {
                ...
                // else if (kp == 34 && CHIM.vowels.indexOf(b) == -1) {
                    //
                    // //console.log(key);
                    // CHIM.ClearBuffer();
                    // CHIM.buffer.push("u");
...
CHIM.Append = function(count, lastkey, key) {
    ...
    //xoa buffer khi co ki tu ngan cach neu khong phai thi push vao buffer
    ...
    if (document.selection) {
        ...
        if (selection.text == "") {
            ...
            // Handle text areas.
                ...
                while (caret.parentElement() == target && caret.move("character", 1) == 1) {--index;
...
CHIM.HTMLEditor.Process = function(target, l) {
    ...
    var idoc = target.parentNode.iframe.contentDocument || target.parentNode.iframe.contentWindow.document;
    var sel = window.getSelection ? idoc.getSelection() : document.selection.createRange();
    var range = CHIM.HTMLEditor.GetRange(target);
...
CHIM.KeyHandler = function(e) {
    ...
    if (e.charCode == null || e.charCode != 0) {// process ASCII only
        ...
        if (keyCode == CHIM.VK_SPACE || keyCode == CHIM.VK_ENTER) {
            ...
            //clear buffer thì sẽ gán lại biến unikey
            CHIM.unikey = false;
        }

        //bo qua các trường hợp sau khi unikey fire unicode
        else if (keyCode > CHIM.VK_SPACE && keyCode < CHIM.VK_LIMIT && !CHIM.unikey) {
            if (CHIM.dirty) {
            ...
            // kiem tra xem co add key ghép được thành chữ cái tiếng việt không
            // nếu có thì updateUI
            if (CHIM.AddKey(key)) {
...
CHIM.Attach = function(e, r) {
    ...
    for ( i = 0; i < f.length; i++) {
        doc = (!window.opera && document.all) ? f[i].contentWindow.document : f[i].contentDocument;
        try {
            doc.iframe = f[i];
            CHIM.Attach(doc, false);
        } catch(e) {
...
CHIM.vn_WW = [65, 258, 193, 7854, 192, 7856, 7842, 7858, 195, 7860, 7840, 7862, 194, 258, 7844, 7854, 7846, 7856, 7848, 7858, 7850, 7860, 7852, 7862, 97, 259, 225, 7855, 224, 7857, 7843, 7859, 227, 7861, 7841, 7863, 226, 259, 7845, 7855, 7847, 7857, 7849, 7859, 7851, 7861, 7853, 7863];
//----------------------------------------------------------------------------
CHIM.vncode_1 = [CHIM.vn_AA, CHIM.vn_EE, CHIM.vn_OO, CHIM.vn_AW, CHIM.vn_OW, CHIM.vn_UW, CHIM.vn_DD, CHIM.vn_WW];
//----------------------------------------------------------------------------
...
Mudim.UpdateUI = function(target, l) {
    ...
    if (target.value) {
        ...
        target.innerHTML = Mudim.HTMLprocess(target, b, start, end);
        //we set the target's content in here
        //and in here we will define the cursor's position
        CHIM.SetCursorPosition(target, start + b.length);
        //only set cursor 's position
        target.scrollTop = t;
    }
};
...
Mudim.FindAccentPos = function(nKey) {
    ...
    default:
        i = p;
        while (i >= 0 && b[i] < CHIM.CHAR_0x80 && CHIM.vowels.indexOf(b[i]) < 0)
        i--;
        //Find the last vowel
        if (i < 0) {
            return -1;
        ...
        while (i - 1 >= 0 && (CHIM.vowels.indexOf(b[i - 1]) >= 0 || b[i - 1] > CHIM.CHAR_0x80) && CHIM.CharPriorityCompare(b[i - 1], b[i]) < 0)
        i--;
        if (i == len - 1 && i - 1 >= 0 && ( j = CHIM.CharIsUI(b[i - 1])) > 0) {
            ...
            case CHIM.CHAR_A:
                ...
                if ((i - 2 < 0 || (j < 24 && b[i - 2] != CHIM.CHAR_q && b[i - 2] != CHIM.CHAR_Q) || (j >= 24 && b[i - 2] != CHIM.CHAR_g && b[i - 2] != CHIM.CHAR_G) ) && (l == 2 || (l == 1 && Mudim.GetMarkTypeID(k, 1) == 1)))
                    i--;
                break;
            case CHIM.CHAR_u:
            case CHIM.CHAR_U:
        ...
        if (i == len - 1 && i - 1 >= 0 && CHIM.CharIsO(b[i - 1]) > 0) {
            ...
            case CHIM.CHAR_a:
            case CHIM.CHAR_A:
                if (!Mudim.newAccentRule && (l == 2 || (l == 1 && Mudim.GetMarkTypeID(k, 1) != 1)))
                    i--;
                break;
            case CHIM.CHAR_e:
            case CHIM.CHAR_E:
                if (!Mudim.newAccentRule)
                    i--;
                break;
            };
        }
...
Mudim.InitPanel = function() {
    if (!Mudim.Panel) {
        Mudim.GetPreference();
        Mudim.panels = ['<div id="mudimPanel" style="position: fixed; bottom: 0; right:0; left:0; width: 100%; border: 1px solid black; padding: 1px; background: ' + Mudim.PANEL_BACKGROUND + '; color:' + Mudim.COLOR + '; z-index:4000; text-align: center; font-size: 10pt;"><a href="http://mudim.googlecode.com" title="Mudzot\'s Input Method" onclick="Mudim.ToggleDisplayMode();return false;">Mudim</a> v0.9 <input name="mudim" id="mudim-off" onclick="Mudim.SetMethod(0);" type="radio">' + Mudim.LANG[0] + '<input name="mudim" id="mudim-vni" onclick="Mudim.SetMethod(1);" type="radio"> ' + Mudim.LANG[1] + ' <input name="mudim" id="mudim-telex" onclick="Mudim.SetMethod(2);" type="radio"> ' + Mudim.LANG[2] + ' <input name="mudim" id="mudim-viqr" onclick="Mudim.SetMethod(3);" type="radio"> ' + Mudim.LANG[3] + ' <input name="mudim" id="mudim-mix" onclick="Mudim.SetMethod(4);" type="radio"> ' + Mudim.LANG[4] + ' <input name="mudim" id="mudim-auto" onclick="Mudim.SetMethod(5);" type="radio"> ' + Mudim.LANG[5] + ' <input id="mudim-checkspell" onclick="javascript:Mudim.ToggleSpeller();" type="checkbox">' + Mudim.LANG[6] + '<input id="mudim-accentrule" onclick="javascript:Mudim.ToggleAccentRule();" type="checkbox">' + Mudim.LANG[7] + ' [&nbsp;<a href="#" onclick="Mudim.Toggle();return false;">' + Mudim.LANG[8] + '</a> (F9) <a href="#" onclick="Mudim.TogglePanel();return false;">' + Mudim.LANG[9] + '</a> (F8) ]</div>', '<div id="mudimPanel" style="position: fixed; bottom: 0; right: 0; width: 120px; border: 1px solid black; padding: 1px; background: ' + Mudim.PANEL_BACKGROUND + '; color:' + Mudim.COLOR + '; z-index:100; text-align: center; font-size: 10pt;"><a href="http://mudim.googlecode.com" title="Mudzot\'s Input Method" onclick="Mudim.ToggleDisplayMode();return false;">Mudim</a>:#METHOD#</div>'];
...
//[position, code, substitution table, index]
/**
 *
 */
Mudim.w = 0;

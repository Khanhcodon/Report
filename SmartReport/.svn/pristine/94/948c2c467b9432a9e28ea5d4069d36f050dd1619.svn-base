...
var port = null,
	...
	maxLengthOfBase64 = 453000,
	maxLengthOfBaseWrite = 1000000,
...
var write_tmp_base64 = {};
function setFileBase64TmpForWrite(writingFileId, data) {
	// Luu lai du lieu de cat thanh nhieu phan khi gui xuong NativeApp
	write_tmp_base64 = write_tmp_base64 || {};
	
	write_tmp_base64[writingFileId] = data;
	write_tmp_base64["origin"] = data;
}

function getFileBase64TmpForWrite(writingFileId) {	
	var tmp = '';
	if(write_tmp_base64[writingFileId].length > maxLengthOfBaseWrite) {
		tmp = write_tmp_base64[writingFileId].substr(0, maxLengthOfBaseWrite);
		write_tmp_base64[writingFileId] = write_tmp_base64[writingFileId].substr(maxLengthOfBaseWrite);
	} else {
		tmp = write_tmp_base64[writingFileId];
		delete write_tmp_base64[writingFileId];
	}
	return tmp;
}

function getFakeData(length) {
    // Thêm một chuỗi bất kỳ để gói tin đúng bằng gói tin chuẩn khi length < maxlength
	length = length - 14; // 14 số byte tên trường fakeData trong chuỗi json
    return new Array(length + 1).join("f");
}
...
function receiveFromContent(message, sender, sendResponse) {    
	...
	if (message.action === Egov_Event.WriteFileBase64 && message.sender.hasAppendModeVersion) {
		// Key dung de danh dau du lieu dang gui nhan cua cung 1 file
		message.writingFileId = (new Date()).getTime() + '';
		setFileBase64TmpForWrite(message.writingFileId, message.data);
		var data = getFileBase64TmpForWrite(message.writingFileId);
		if (data.length < maxLengthOfBaseWrite) {
		    message.fakeData = getFakeData(maxLengthOfBaseWrite - data.length);
		}
		message.data = data;
	}

	if (message.action == Egov_Event.HasAppendModeVersion) {
	    message.sender.IsCheckModeVersion = true;
	    message.action = Egov_Event.WriteFileBase64;
	    message.writingFileId = (new Date()).getTime() + '';
	}
...
function receiveFromNativeApp(message) {	
	...
	*/
    ...
    // Kiểm tra plugin cũ có hỗ trợ append mode ko
	var writingFileId = message.writingFileId;
	if (writingFileId == null && message.sender.IsCheckModeVersion == true) {
	    message.hasAppendWriteMode = false;
	    sendToContent(message);
	    return;
	}

	message.hasAppendWriteMode = true;
    // Kiểm tra phiên bản của native app
	if (message.action === Egov_Event.WriteFileBase64 && message.returnCode == 1
             && message.sender.hasAppendModeVersion && writingFileId && write_tmp_base64[writingFileId]) {
	    // Nếu chưa đọc hết nội dung file => Tiếp tục đọc

	    var data = getFileBase64TmpForWrite(writingFileId);
		var message_write = {
			"action": Egov_Event.WriteFileBase64,
			"fileName": message.fileName,
			"data": data,
			"writingFileId": writingFileId,
			"sender": message.sender || {}
		};

		if (data.length < maxLengthOfBaseWrite) {
		    message_write.fakeData = getFakeData(maxLengthOfBaseWrite - data.length);
		}

		// Send lại sang NativeApp để lấy nội dung Base64
		sendToNativeApp(message_write);
		return;
	}
